...
def Mk_tpTable(vmdir,vm_list):
    ...
    depucor = np.zeros((nray0,para_vel['nd0']))
    deptcor = np.zeros((nray0,para_vel['nd0']))
    tt = np.zeros((para_vel['nx0'],para_vel['nd0'],para_vel['nindex']))
    xsave = np.zeros(20000)
    tsave = np.zeros(20000)
    psave = np.zeros(20000)
    usave = np.zeros(20000)
        ...
        for idep in range(0,ndep):
            ...
            for idel in np.arange(0,ndel):
                ...
                for i in np.arange(1,ncount):
                    ...
                    frac = (dell-x1)/(x2-x1)
                    t1 = tsave[i-1]+frac*(tsave[i]-tsave[i-1])
                    if t1<tt[idel,idep,itab]:
                        tt[idel,idep,itab] = t1
                        scr1 = psave[i]/usave[i]
                        angle = np.arcsin(scr1)/np.pi*180
                        if angle<0:
                            angle = -angle
    ...
    return table,tt,deptab,delttab
################################################
def Mk_tsTable(vmdir,vm_list):
    ...
    return table,tt,deptab,delttab
#####################################################################
#      Functions for interpolating travel time   #
#####################################################################
def Get_TTS_tt(dist,qdep,table,deptab,delttab):
    if qdep<deptab[0]:
        qdep=deptab[0]
    if qdep>deptab[-1]:
        qdep = deptab[-1]
    ...
    if table[ix1,id1]==0 or table[ix1,id2]==0 or \
        table[ix2,id2]==0 or table[ix2,id1]==0 or delttab[ix2]<dist:
    # Extrapolate to get
        iflag = 1
        xoffmin1 = 999
        xoffmin2 = 999
        ...
        for ix in np.arange(1,len(delttab)):
            if table[ix-1,id1]==0 or table[ix,id1]==0:
                if table[ix-1,id2]==0 or table[ix,id2]==0:
                    continue
...
#####################################################################
#      Functions for interpolating take-off angle   #
#####################################################################
